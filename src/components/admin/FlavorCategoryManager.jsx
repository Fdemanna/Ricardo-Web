...
export default function FlavorCategoryManager() {
    const { data: rawCategories, loading, addItem: addCategory, updateItem: updateCategory, deleteItem: deleteCategory } = useFirebaseCollection('categories', null);
    ...
    return (
        <div className="bg-white rounded-custom p-8 border border-chocolate/5 shadow-sm mt-12 mb-12">
            <div className="mb-6 border-b border-chocolate/10 pb-4">
                ...
                <p className="text-sm text-chocolate/70">Añade o edita las categorías predefinidas que aparecerán al crear nuevos sabores. Arrastra para reordenar cómo aparecen en la carta.</p>
            </div>
            ...
            <div className="flex flex-col gap-2">
                {categories.length === 0 ? (
                    <p className="text-sm text-chocolate/50 italic bg-cream/50 p-4 rounded-md text-center">Todavía no hay categorías establecidas.</p>
                ) : (
